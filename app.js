  ...
  window.startFight = function(){
    ...
    function endRound(winner){
      ...
      if(p1R>=BEST || p2R>=BEST){
        ...
        if(p1Win){
          const wins=Number(localStorage.getItem('spider_fight_wins')||0)+1; localStorage.setItem('spider_fight_wins',String(wins));
    ...
    function overlay(){
      if(intro>0){ ctx.fillStyle='rgba(0,0,0,.55)'; ctx.fillRect(0,0,W,H); ctx.fillStyle=intro>31?'#ffd700':'#00ffc8'; ctx.shadowColor=ctx.fillStyle; ctx.shadowBlur=28; ctx.font="900 37px 'Orbitron',monospace"; ctx.textAlign='center'; ctx.fillText(intro>31?'ROUND '+round:'FIGHT!',W/2,H/2+10); ctx.shadowBlur=0; }
