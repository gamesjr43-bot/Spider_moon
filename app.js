...
async function forumLoadTopics(){
  ...
  // search bar
  const searchBar = document.getElementById("forumTopicSearchBar");
  if(!searchBar){
    const bar = document.createElement("div");
    bar.id = "forumTopicSearchBar";
    bar.className = "forum-tools-v2";
    bar.innerHTML = `<input id="forumSearchInput" placeholder="🔍 Buscar tópico..." oninput="forumApplySearch()" style="font-size:13px">`;
...
window.forumShowTopics = function(){
  ...
  forumSearchTerm="";
  const bar=document.getElementById("forumTopicSearchBar"); if(bar) bar.remove();
...
window.adminShowReports = async function(){
  ...
  adminHideAll();
  let sec = document.getElementById("adminReportsSection");
  if(!sec){
    sec = document.createElement("div");
    sec.id = "adminReportsSection";
    sec.style.cssText = "margin-top:14px";
    document.querySelector("#admin .panel-content, #admin")?.appendChild(sec);
  }
  sec.style.display="block";
  const q = query(collection(db,"reports"),orderBy("createdAt","desc"),limit(40));
  const snap = await getDocs(q);
  if(snap.empty){ sec.innerHTML=`<p style="color:var(--text-dim);text-align:center;margin-top:20px">Nenhuma denúncia.</p>`; return; }
  sec.innerHTML = `<h3 style="font-family:var(--font-display);color:var(--primary);margin-bottom:12px">📋 Denúncias</h3>` +
    ...
    <div style="background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:8px">
      <div style="font-size:12px;color:var(--text-dim);margin-bottom:4px">Tipo: <b>${sanitizeHTML(r.type)}</b> · ID: ${sanitizeHTML(r.refId)} · por ${sanitizeHTML(r.reporter)} · ${formatDate(r.createdAt)}</div>
      <div style="font-size:13px;color:var(--text)">${sanitizeHTML(r.reason)}</div>
      <button onclick="adminDismissReport('${d.id}')" class="danger" style="font-size:11px;padding:5px 10px;margin-top:8px">Dispensar</button>
    </div>`; }).join("");
};
